...
public class AtributoParserTest extends TestCase {
	...
	private static final String PATH_1_ATRIBUTO_PARSER_TEST = "src/mereditor/tests/xml de prueba/atributo-comp.xml";
	private static final String PATH_2_ATRIBUTO_PARSER_TEST = "src/mereditor/tests/xml de prueba/atributoCompuesto-comp.xml";
	...
	public void testParsearAtributoSimple() throws Exception {
		...
		Element atributoSimpleXML= (Element) raiz.getElementsByTagName( AtributoParser.ATRIBUTO_TAG ).item(0);
		
		/*Inicializo el atributo con el que voy a comparar el parseado*/
		Atributo atributoSimple= new Atributo (NOMBRE_ATTR, ID_ATTR, ID_DIAGRAMA, CARD_MIN, CARD_MAX, TIPO_ATTR, null);
		
		/*LLamado al metodo */
		AtributoParserFake parser= new AtributoParserFake ();
	...
	public void testParsearAtributoCompuesto() throws Exception{
		...
		Document doc= builder.parse(source);
		Element raiz= doc.getDocumentElement();
		Element atributoCompuestoXML = (Element) raiz.getElementsByTagName(AtributoParser.ATRIBUTO_TAG).item(0);
		...
		assertTrue ( parser.getAtributosContenidos().get(0).getIdComponente().equals( "1" ) );
		assertTrue ( parser.getAtributosContenidos().get(1).getIdComponente().equals( "2" ) );
		assertTrue ( parser.getAtributosContenidos().get(4).getIdComponente().equals( "5" ) );
